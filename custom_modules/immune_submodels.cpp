...
void immune_cell_motility_direction( Cell* pCell, Phenotype& phenotype , double dt )
{
	...
	std::vector<double> gradC = pCell->nearest_gradient(ISF_index);
	normalize( &gradC ); 
	gradC *= pCell->custom_data["sensitivity_to_ISF_chemotaxis"];
...
void CD4_Tcell_phenotype( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	// test antibody 9-16-2021 !!!
	static int Ig_index = microenvironment.find_density_index( "Ig" );
	pCell->phenotype.secretion.secretion_rates[Ig_index] = 1.0;
...
void immune_cell_recruitment_LNP( double dt )
{
	static int proinflammatory_cytokine_index = microenvironment.find_density_index("pro-inflammatory cytokine");	

	static int epitope_external = microenvironment.find_density_index( "epitope" ); 

	static double dt_immune = parameters.doubles( "immune_dt" );  // 10 min
	...
	if( t_immune > t_next_immune- tolerance )
	{
		...
		// CD8 Tcell recruitment in mRNA vaccine project
		double total_antigenic_epitope = 0;
		...
		for( int n=0; n < (*all_cells).size() ; n++ )
		{
			...
			{
				total_antigenic_epitope += pCell->custom_data[epitope_index];
			}
		}

		// expected number of  CD8 T
		double number_of_new_cells_prob = UniformRandom()*total_antigenic_epitope * elapsed_time* 0.01; // slow down ;
		...
		if( number_of_new_cells_int>2 )
		{
			number_of_new_cells_int = 2;   // try to avoid significantly CD8 T
		}
		...
		if( number_of_new_cells_int )
		{
			...
			std::cout << "\tRecruiting " << number_of_new_cells_int << " CD8 T... " << std::endl; 
			
			for( int n = 0; n < number_of_new_cells_int ; n++ )
			{ create_infiltrating_Tcell(); }   // CD8 T cell
		}



		// CD4 Tcell recruitment in mRNA vaccine project

		// expected number of  CD4 T
		number_of_new_cells_prob = UniformRandom()*total_antigenic_epitope * elapsed_time * 0.01; // slow down 
		...
		recruited_CD4Tcells += number_of_new_cells_int;

		if( number_of_new_cells_int>2 )
		{
			number_of_new_cells_int = 2;   // try to avoid significantly CD4 T
		}
		...
		if( number_of_new_cells_int )
		{
			...
			std::cout << "\tRecruiting " << number_of_new_cells_int << " CD4 T... " << std::endl; 
			
			for( int n = 0; n < number_of_new_cells_int ; n++ )
			{ create_infiltrating_CD4Tcell(); }   // CD4 T cell
		}



		// antibody recruitment
		int total_CD4_Tcell = 0;
		...
		{
			Cell* pCell = (*all_cells)[n];
			if( pCell->phenotype.death.dead == false && pCell->type == CD4_type )
			{
				total_CD4_Tcell += 1;
			}
		}

		double number_of_new_Ig_prob = UniformRandom()*total_CD4_Tcell * elapsed_time * 0.01; // slow down 
		double number_of_new_Ig_int = floor(number_of_new_Ig_prob);
		
		static double x_min = microenvironment.mesh.bounding_box[0] + 1e-6; 
		static double x_max = microenvironment.mesh.bounding_box[3] - 1e-6; 
		static double y_min = microenvironment.mesh.bounding_box[1] + 1e-6; 
		static double y_max = microenvironment.mesh.bounding_box[4] - 1e-6; 
		
		static int nAb = microenvironment.find_density_index( "Ig" ); 
		for( int n=0 ; n < number_of_new_Ig_int ; n++ )
			{
				// pick a random voxel 
				std::vector<double> position = {0,0,0}; 
		...
		if (number_of_new_Ig_int > 0)
		{ std::cout << "\tRecruiting " << number_of_new_Ig_int << " Ig... " << std::endl; }
		


		// ======================== old recruitment functions ============================= //

		// macrophage recruitment 
		static double macrophage_recruitment_rate = parameters.doubles( "macrophage_max_recruitment_rate" ); 
		static double M_min_signal = 0.00000001*parameters.doubles( "macrophage_recruitment_min_signal" ); // !!!!!!!!!!!!
		...
		for( int n=0; n<microenvironment.mesh.voxels.size(); n++ )
		{
			...
			double dRate = ( microenvironment(n)[epitope_external] - M_min_signal ); // !!!!!!!!!!!!
		...
		if( number_of_new_cells_int )
		{
			...
			for( int n = 0; n < number_of_new_cells_int ; n++ )
			{ create_infiltrating_macrophage(); }   // macrophage
		}
